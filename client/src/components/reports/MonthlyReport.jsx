...
import { getMonthlyReport, exportReportPDF, exportReportCSV } from '../../services/reportService';
import ReportFilters from './ReportFilters';
import LoadingSpinner from '../common/LoadingSpinner';

const MonthlyReport = ({ departments = [] }) => {
  ...
  const handleExport = (format, exportFilters) => {
    if (format === 'pdf') {
      exportReportPDF('monthly', exportFilters);
    } else if (format === 'csv') {
      exportReportCSV('monthly', exportFilters);
    }
  };
  ...
  const { statistics, tasks, month, year, startDate, endDate } = reportData;
  const monthNames = [
        ...
        <div className="p-6">
          ...
          <div>
            ...
            {tasks.length > 0 ? (
              ...
              <p className="text-gray-500 text-center py-4">No tasks found for this month.</p>
            )}
          </div>
        </div>
      </div>
