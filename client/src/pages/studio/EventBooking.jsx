...
import { getEquipmentOptions } from '../../services/studioService';
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        if (isEditing && editId) {
          setLoading(true);
          const eventResponse = await getEvent(editId);
          if (eventResponse.success && eventResponse.data) {
            const event = eventResponse.data;
            setFormData({
              client: event.client?._id || '',
              useExistingClient: !!event.client,
              eventType: event.eventType || '',
              eventTitle: event.eventTitle || '',
              eventDescription: event.eventDescription || '',
              bookingDate: event.bookingDate ? new Date(event.bookingDate).toISOString().split('T')[0] : '',
              timeSlot: {
              ...
              equipment: event.equipment?.map(eq => eq.equipmentId || eq._id) || [],
              specialRequirements: event.specialRequirements || '',
              cateringNeeded: event.cateringNeeded || false,
              cateringDetails: event.cateringDetails || '',
              photographyNeeded: event.photographyNeeded || false,
              videographyNeeded: event.videographyNeeded || false,
              liveStreamingNeeded: event.liveStreamingNeeded || false,
              estimatedBudget: event.estimatedBudget || '',
              additionalNotes: event.additionalNotes || ''
            });
          } else {
            // Handle different error types
            if (eventResponse.isDemoData) {
              toast.error('Cannot edit demo data. Please create a real event booking first.');
            } else if (eventResponse.isInvalidId) {
              toast.error('Invalid event ID. Cannot edit demo data.');
            } else if (eventResponse.notFound) {
              toast.error('Event not found. It may have been deleted.');
            } else {
              toast.error(eventResponse.error || 'Failed to load event data for editing');
            }
            navigate('/events');
          }
        }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!formData.eventTitle || !formData.bookingDate || !formData.timeSlot.startTime || 
          (!formData.useExistingClient && !formData.contactPerson.name) ||
      ...
      // Prepare event data
      const eventData = {
        client: formData.useExistingClient ? formData.client : null,
        contactPerson: {
          name: formData.contactPerson.name,
          phone: formData.contactPerson.phone,
          email: formData.contactPerson.email,
          company: formData.contactPerson.company
        },
        ...
        purpose: formData.eventTitle ? `${formData.eventType}: ${formData.eventTitle}` : (formData.eventType || 'Event'),
        equipment: formData.equipment.map(equipmentId => {
          const equipment = equipmentOptions.find(eq => eq.id === equipmentId);
          return {
            equipmentId: equipment?.id,
            name: equipment?.name || 'Equipment',
            quantity: 1,
            rate: equipment?.dailyRate || equipment?.price || 0
          };
        }),
        specialRequirements: formData.specialRequirements,
        cateringNeeded: formData.cateringNeeded,
        cateringDetails: formData.cateringDetails,
        photographyNeeded: formData.photographyNeeded,
        videographyNeeded: formData.videographyNeeded,
        liveStreamingNeeded: formData.liveStreamingNeeded,
      ...
      if (response.success) {
        toast.success(`Event ${isEditing ? 'updated' : 'created'} successfully! We will contact you shortly.`);
        navigate('/events');
      } else {
        toast.error(response.error || `Failed to ${isEditing ? 'update' : 'create'} event`);
      }
    } catch (error) {
    ...
    <div className="max-w-4xl mx-auto">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900">
          {isEditing ? 'Edit Event Booking' : 'Event Booking'}
        </h1>
        <p className="text-gray-600 mt-2">
          {isEditing 
            ? 'Update the event booking details below.'
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        {/* Contact Information - Only show if not using existing client */}
        {!formData.useExistingClient && (
          <div className="bg-white rounded-lg shadow p-6">
          ...
          <div className="bg-blue-50 rounded-lg p-6">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                <span>Equipment Services ({formData.equipment.length} items):</span>
        ...
        <div className="flex justify-end space-x-4">
          ...
          >
            ...
            {isEditing ? 'Update Event' : 'Submit Booking Request'}
          </button>
        </div>
      </form>
    </div>
