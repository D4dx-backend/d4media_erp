...
const { getSystemInfo, getDatabaseStatus, performHealthCheck } = require('./src/utils/network');

// Clustering for production environment
const isMaster = !process.env.PM2_CLUSTER && cluster.isMaster && process.env.NODE_ENV === 'production';
...
if (isMaster) {
  ...
  cluster.on('exit', (worker, code, signal) => {
    appLogger.warn(`Worker ${worker.process.pid} died with code ${code} and signal ${signal}`);
...
app.get('/api/v1/system', async (req, res) => {
  ...
  if (process.env.NODE_ENV === 'production' && (!req.user || req.user.role !== 'super_admin')) {
