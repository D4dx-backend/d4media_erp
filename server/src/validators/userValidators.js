...
const createUserValidation = [
  ...
  body('department')
    .optional({ nullable: true, checkFalsy: true })
    .custom((value, { req }) => {
      // Skip validation if no value or if user is a client
      if (!value || req.body.role === 'client') return true;
...
const getUsersValidation = [
  ...
  query('role')
    ...
    .custom(value => {
      ...
      // Check if it's a comma-separated list of valid roles
      const roles = value.split(',');
      const validRoles = ['super_admin', 'department_admin', 'department_staff', 'reception', 'client'];
