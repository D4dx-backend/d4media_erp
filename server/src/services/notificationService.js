...
class NotificationService {
  ...
  async createNotification(data) {
    try {
      ...
      if (this.io) {
        ...
        // Update unread count
        const unreadCount = await this.getUnreadNotificationsCount(data.recipient);
  ...
  async getUserNotifications(userId, options = {}) {
    try {
      ...
      return { notifications: [], pagination: { total: 0, limit: 0, skip: 0, hasMore: false } };
    }
  }
  ...
  async sendTaskAssignmentNotification(task, assignedUser) {
    try {
      ...
      await this.createNotification({
        ...
        message: `You have been assigned a new ${task.priority} priority task due on ${new Date(task.dueDate).toLocaleDateString()}.`,
      ...
      for (const user of stakeholders) {
        ...
        const html = `
          ...
          <div style="border: 1px solid #ddd; padding: 15px; margin: 10px 0;">
            ...
            <p><strong>New Status:</strong> ${this.formatStatus(task.status)}</p>
        ...
        await this.createNotification({
          ...
          message: `Status changed from ${this.formatStatus(previousStatus)} to ${this.formatStatus(task.status)} by ${changedBy.name}.`,
      ...
      for (const user of stakeholders) {
        ...
        const html = `
          ...
          <div style="border: 2px solid #dc3545; padding: 15px; margin: 10px 0; background-color: #f8d7da;">
            ...
            <p><strong>Priority:</strong> ${task.priority}</p>
            <p><strong>Current Status:</strong> ${this.formatStatus(task.status)}</p>
            <p><strong>Progress:</strong> ${task.progress.percentage}%</p>
          </div>
      ...
      for (const user of stakeholders) {
        ...
        const html = `
          ...
          <div style="border: 2px solid #ffc107; padding: 15px; margin: 10px 0; background-color: #fff3cd;">
            <h3>${task.title}</h3>
            <p><strong>Due Date:</strong> ${new Date(task.dueDate).toLocaleDateString()} at ${new Date(task.dueDate).toLocaleTimeString()}</p>
  ...
  async sendProgressUpdateNotification(task, progressNote, updatedBy) {
    try {
      ...
      for (const user of stakeholders) {
        if (!user.notifications?.taskUpdates || user._id.toString() === updatedBy._id.toString()) continue;
        ...
        const html = `
          ...
          <div style="border: 1px solid #ddd; padding: 15px; margin: 10px 0;">
            ...
            ${progressNote ? `<p><strong>Note:</strong> ${progressNote}</p>` : ''}
          </div>
          <p>Log in to the system to view more details.</p>
        `;
  ...
  async sendClientFeedbackNotification(task, feedback, client) {
    try {
      ...
      for (const user of stakeholders) {
        if (!user.notifications?.taskUpdates || user.role === 'client') continue;
        ...
        await this.createNotification({
          ...
          message: `${client.name} has provided feedback: "${feedback.substring(0, 100)}${feedback.length > 100 ? '...' : ''}"`,
          priority: 'high',
          relatedModel: 'Task',
          relatedId: task._id,
          metadata: {
  ...
  async getTaskStakeholders(task) {
    ...
    if (task.department && task.department.admin) {
      if (typeof task.department.admin === 'object' && task.department.admin._id) {
  ...
  async checkAndSendDeadlineReminders() {
    try {
      ...
      for (const task of tasksDueIn24Hours) {
        const hoursUntilDeadline = Math.ceil((new Date(task.dueDate) - now) / (1000 * 60 * 60));
      ...
      console.log(`Sent deadline reminders for ${tasksDueIn24Hours.length + tasksDueIn2Hours.length} tasks`);
    } catch (error) {
